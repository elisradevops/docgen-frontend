    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.invalidFiles && result.invalidFiles.length > 0) {
          toast.warning(
            `${result.invalidFiles.length} file(s) skipped due to invalid docType: ${result.invalidFiles.map(f => f.name).join(', ')}. Valid types are: STD, STP, STR, SVD, SRS, SYSRS, Meeting-Summary`,
            { autoClose: 8000 }
          );
        }
